...
using NetCoreUI.Bridge.Api;
using NetCoreUI.Bridge.Models;
...
namespace NetCoreUI.Bridge
{
    internal class WebviewBridge : IWebviewBridge
    {
        ...
        private string GetInvokeScript(string id, object data)
        {
            ...
            return $"window._NetCoreUI._sendEvent({idJson}, {dataJson})";
        }
        ...
        private async Task EndApiCall(InvokeInfoModel info, ApiResultModel result)
        {
            ...
            string script = $"window._NetCoreUI._endApiCall({info.CallbackId}, {resultJson})";
